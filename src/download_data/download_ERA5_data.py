...
def download_raw_data(options, file_name):
    ...
    download_ERA5_structure = {
        ...
        'time': ['0' + str(t) + ':00' if t < 10 else str(t) + ':00'
                 for t in range(options['start_time'], options['end_time'] + 1, options['step_time'])],
...
def download_and_preprocessing_ERA5_data(config):
    config.download_ERA5_options['work_dir'].mkdir(parents=True, exist_ok=True)

    start_year = config.download_ERA5_options['start_year']
    end_year = config.download_ERA5_options['end_year']
    middle_year = (start_year + end_year) // 2
    
    print('Downloading data from cds.climate.copernicus.eu...')
    # download the first part
    config.download_ERA5_options['end_year'] = middle_year
    file_name_1st = config.download_ERA5_options['work_dir'] / (config.download_ERA5_options['name_var'] + '_' +
                                                                str(start_year) + '_' + str(middle_year) + '.nc')
    download_raw_data(config.download_ERA5_options, file_name_1st)
    
    # download the second part
    config.download_ERA5_options['start_year'] = middle_year + 1
    config.download_ERA5_options['end_year'] = end_year
    file_name_2nd = config.download_ERA5_options['work_dir'] / (config.download_ERA5_options['name_var'] + '_' +
                                                                str(middle_year + 1) + '_' + str(end_year) + '.nc')
    download_raw_data(config.download_ERA5_options, file_name_2nd)
    config.download_ERA5_options['start_year'] = start_year
    print('Done\n')
    
    # npz cube formation
    print('Forming the resulting data cube...')
    dset_1 = xr.open_dataset(str(file_name_1st))
    data_cube_1 = read_nc.get_data_cube(dset_1, config.download_ERA5_options['name_var'])
    dset_2 = xr.open_dataset(str(file_name_2nd))
    data_cube_2 = read_nc.get_data_cube(dset_2, config.download_ERA5_options['name_var'])
    ...
    np.savetxt(config.download_ERA5_options['work_dir'] / config.download_ERA5_options['lon_file_name'],
               longitude, fmt='%.2f')
    np.savetxt(config.download_ERA5_options['work_dir'] / config.download_ERA5_options['lat_file_name'],
               latitude, fmt='%.2f')
    
    times = read_nc.form_times(config.download_ERA5_options)
    print("n_times = ", len(times))
    np.savetxt(config.download_ERA5_options['work_dir'] / config.download_ERA5_options['times_file_name'],
               times, fmt='%s')
    ...
    np.savez(config.download_ERA5_options['work_dir'] / config.download_ERA5_options['res_cube_file_name'],
             resulting_cube)
    print('Done\n')
    
    preprocessing_data(config.download_ERA5_options, resulting_cube, latitude, longitude, times)
