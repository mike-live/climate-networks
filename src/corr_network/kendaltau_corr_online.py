...
class kendalltau_corr_online:
    ...
    def popcount(self, x):
        return self.popcnt[x & (self.popcnt_mask)] + self.popcnt[(x >> np.uint64(16)) & (self.popcnt_mask)] + self.popcnt[(x >> np.uint64(32)) & (self.popcnt_mask)] + self.popcnt[(x >> np.uint64(48)) & (self.popcnt_mask)]
    ...
    def precompute_masks(self):
        ...
        mask_left_less = np.zeros((nm, nt), dtype = np.uint64)
        mask_left_greater = np.zeros((nm, nt), dtype = np.uint64)
        mask_left_equal = np.zeros((nm, nt), dtype = np.uint64)

        mask_right_less = np.zeros((nm, nt), dtype = np.uint64)
        mask_right_greater = np.zeros((nm, nt), dtype = np.uint64)
        mask_right_equal = np.zeros((nm, nt), dtype = np.uint64)
    ...
    def init(self, i, j, ti, tj):
        ...
        for k in range(self.window):
            if self.ti + k >= nt or self.tj + k >= nt:
                break
            self.add_pair(i, j, self.ti + k, self.tj + k, np.uint64((1 << k) - 1))
    ...
    def add_pair(self, i, j, ti, tj, subset = np.iinfo(np.uint64).max):
        self.dis += self.popcount(self.mask_left_less   [i, ti] & self.mask_left_greater[j, tj] & subset)
        self.dis += self.popcount(self.mask_left_greater[i, ti] & self.mask_left_less   [j, tj] & subset)
        self.ntie += self.popcount(self.mask_left_equal [i, ti] & self.mask_left_equal  [j, tj] & subset)

    def remove_pair(self, i, j, ti, tj):
        self.dis -= self.popcount(self.mask_right_less   [i, ti] & self.mask_right_greater[j, tj])
        self.dis -= self.popcount(self.mask_right_greater[i, ti] & self.mask_right_less   [j, tj])
        self.ntie -= self.popcount(self.mask_right_equal [i, ti] & self.mask_right_equal  [j, tj])
...
def compute_tau_kendall_overall_online(tau_corr, data, ids, window_size = 15 * 4, delay_time = 7 * 4, alpha = 0.05):
    #print(ids[0], len(ids))
    nm, nt = data.shape
